...
### 1. `tape/tape.go`
- [ ] Adicionar `func (t *Tape) MarkPlayed(playedAt time.Time)` (seta
      `played = true`, `playedAt = &playedAt`) — simétrico ao
      `SetOpeningTime` de `Mailbox`.
- [ ] Adicionar getters: `MailboxID()`, `RecordedBy()`, `RecordedAt()`,
      `Played()`, `PlayedAt()`.
- [ ] Adicionar `type Repository interface` com `Save(ctx, *Tape) error`,
      `FindByID(ctx, uuid.UUID) (*Tape, error)`,
      `ListByMailboxID(ctx, uuid.UUID) ([]*Tape, error)`.
- [ ] Definir `ErrNotFound` sentinel para o repositório sinalizar "não
      encontrado".

### 2. `mailbox/mailbox.go`
- [ ] Adicionar getters: `OwnerID()`, `CreatedAt()`, `LastListenedAt()`,
      `OpensAt()`.
- [ ] Adicionar `type Repository interface` com `Save(ctx, *Mailbox) error`,
      `FindByID(ctx, uuid.UUID) (*Mailbox, error)`.
- [ ] Definir `ErrNotFound` sentinel.
